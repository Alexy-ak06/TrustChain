...
export default function MiningSimulation() {
  ...
  // Gatekeeper reference to prevent audio loops
  const lastVerifiedBlockRef = useRef(1284);

  useEffect(() => {
    // 🎙️ System initialization voice broadcast
    if (typeof speakTrustChain === "function") {
      window.speechSynthesis.cancel();
      speakTrustChain("Mining simulation node online. Initializing block validation loops.");
    }

    const interval = setInterval(() => {
      // Smooth incremental nonce updates
      const nonceIncrement = Math.floor(Math.random() * 450) + 50;
      setNonce((prev) => prev + nonceIncrement);
      
      // Fluid floating-point hash rate fluctuation
      const dynamicHash = (92.4 + Math.random() * 12.8).toFixed(2);
      setHashRate(parseFloat(dynamicHash));

      // 🎲 Synchronized block discovery event engine
      if (Math.random() > 0.72) {
        setBlocks((prev) => {
          const nextBlock = prev + 1;
          
          // GATED VOICE TRIGGER: Only speaks if it's a new, unannounced block
          if (nextBlock > lastVerifiedBlockRef.current) {
            if (typeof speakTrustChain === "function") {
              window.speechSynthesis.cancel();
              speakTrustChain(`Block ${nextBlock} verified. Appending state ledger changes.`);
            }
            lastVerifiedBlockRef.current = nextBlock;
          }

          // Push context-aware block logs
          setLogs((prevLogs) => [
        ...
        const randomLog = routineEvents[Math.floor(Math.random() * routineEvents.length)];
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 mt-16">
          ...
          <div className="bg-cyan-950/5 border border-cyan-500/10 rounded-2xl p-6 backdrop-blur-md">
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Current Nonce</p>
            <h3 className="text-3xl md:text-4xl font-black text-cyan-400 mt-4 tracking-tight">{nonce.toLocaleString()}</h3>
            <div className="h-[3px] w-full bg-cyan-950 rounded-full mt-6 overflow-hidden">
              <motion.div className="h-full bg-cyan-400" animate={{ width: `${(nonce % 100)}%` }} transition={{ duration: 0.3 }} />
            </div>
          </div>

          {/* HASH RATE PANEL */}
          <div className="bg-emerald-950/5 border border-emerald-500/10 rounded-2xl p-6 backdrop-blur-md">
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Hash Throughput</p>
            <h3 className="text-3xl md:text-4xl font-black text-emerald-400 mt-4 tracking-tight">
              {hashRate} <span className="text-xs text-emerald-600 font-normal">TH/s</span>
            </h3>
            <div className="h-[3px] w-full bg-emerald-950 rounded-full mt-6 overflow-hidden">
              <motion.div className="h-full bg-emerald-400" animate={{ width: `${((hashRate - 92) / 13) * 100}%` }} transition={{ duration: 0.4 }} />
            </div>
          </div>

          {/* BLOCKS PANEL */}
          <div className="bg-purple-950/5 border border-purple-500/10 rounded-2xl p-6 backdrop-blur-md sm:col-span-2 md:col-span-1">
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Verified Blocks</p>
            <h3 className="text-3xl md:text-4xl font-black text-purple-400 mt-4 tracking-tight">{blocks}</h3>
            <div className="h-[3px] w-full bg-purple-950 rounded-full mt-6 overflow-hidden">
              <motion.div className="h-full bg-purple-400" animate={{ width: "100%" }} key={blocks} initial={{ width: "0%" }} transition={{ duration: 1, ease: "easeOut" }} />
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 bg-black/60 border border-cyan-500/10 rounded-2xl p-6 backdrop-blur-xl shadow-2xl">
          <div className="flex items-center justify-between border-b border-cyan-500/10 pb-4">
            <h4 className="text-sm font-black text-white uppercase tracking-widest">Live Core Verification Output</h4>
            <div className="flex items-center gap-2 text-emerald-400 text-xs">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="font-bold tracking-wider">SECURE GRID LINK</span>
            </div>
          </div>
